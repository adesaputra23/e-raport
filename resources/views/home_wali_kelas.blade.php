<div class="row" style="margin-top: -14px;">
    <div class="col-xl-6 col-md-6">
        <a href="{{ URL(Session::get('prefix') . '/siswa/lihat-data') }}">
            <div class="card">
                <div class="card-body">
                    <div class="card-body border border-primary">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <p class="text-truncate font-size-14 mb-2" style="color: black;">Jumlah Siswa</p>
                                <h4 class="mb-2">{{ $count_siswa }}</h4>
                            </div>
                            <div class="avatar-sm">
                                <span class="avatar-title bg-light text-primary rounded-3">
                                    <i class="fas fa-user-graduate font-size-24"></i>
                                </span>
                            </div>
                        </div>
                    </div>
                </div><!-- end cardbody -->
            </div><!-- end card -->
        </a>
    </div>
    <div class="col-xl-6 col-md-6">
        <a href="{{ URL(Session::get('prefix') . '/mata-pelajaran') }}">
            <div class="card">
                <div class="card-body">
                    <div class="card-body border border-primary">
                        <div class="d-flex">
                            <div class="flex-grow-1">
                                <p class="text-truncate font-size-14 mb-2" style="color: black;">Jumlah Mata Pelajaran
                                </p>
                                <h4 class="mb-2">{{ $count_mapel }}</h4>
                            </div>
                            <div class="avatar-sm">
                                <span class="avatar-title bg-light text-primary rounded-3">
                                    <i class="fas fa-list-alt font-size-24"></i>
                                </span>
                            </div>
                        </div>
                    </div>
                </div><!-- end cardbody -->
            </div><!-- end card -->
        </a>
    </div>
</div>
